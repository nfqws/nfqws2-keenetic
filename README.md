...
Записи различаются даже у соседних поддоменов одной зоны — проверено 17.09.2026,
3 запроса на домен:

| Домен | HTTPS RR | `ech` |
| --- | --- | --- |
| `cloudflare.com` | есть | нет |
| `developers.cloudflare.com` | есть | нет |
| `research.cloudflare.com` | есть | есть |

Поэтому проверять нужно тот домен, к которому реально обращается клиент, а не тот, что
виден в адресной строке: браузер и десктопное приложение одного сервиса часто ходят на
разные поддомены. Для доменов с `ech` настройка по имени неприменима; остаётся работа
по адресам (`ipset.list`) либо отключение ECH на стороне клиента, если он это позволяет.
