...
### Проверка работоспособности с самого роутера

`/opt/etc/nfqws2/health-check.sh` качает содержимое по списку целей, считает полученные
байты и сравнивает с сохранённым эталоном. Смысл запускать именно на роутере: компьютер
включён не всегда, а роутер работает круглосуточно, и его собственный трафик проходит
через тот же NFQUEUE, что и клиентский.

```bash
/opt/etc/nfqws2/health-check.sh --save-baseline   # запомнить текущие значения как эталон
/opt/etc/nfqws2/health-check.sh --runs 3          # проверить (по умолчанию 3 прогона)
/opt/etc/nfqws2/health-check.sh --quiet           # только в лог, для cron
```

Цели перечисляются в `/opt/etc/nfqws2/health-targets.list`, эталон хранится рядом в
`health-baseline.tsv`, лог — `/opt/var/log/nfqws2-health.log`. Код возврата 1, если
хоть одна цель отдала ноль байт или меньше половины эталона — годится для cron с
уведомлением.

Берётся лучший результат из нескольких прогонов: единичная неудача — это шум, реальная
поломка проваливает все прогоны подряд.

**Чего эта проверка не видит.** Она открывает по одному короткому соединению на запрос,
потому что `wget` закрывает его сразу после ответа. Есть класс поломок, когда короткие
запросы проходят полностью, а одно долгое keep-alive соединение встаёт после первого
ответа — страница тогда открывается без стилей и картинок. Зелёный результат этой
проверки такую поломку не исключает.
