...
### Автообновление `ipset.list`

Адреса некоторых сервисов меняются достаточно часто, чтобы список, собранный руками,
устаревал за считанные дни. Скрипт `/opt/etc/nfqws2/ipset-update.sh` пересобирает
`ipset.list` по данным [iplist.opencck.org](https://iplist.opencck.org) для сервисов,
перечисленных в `/opt/etc/nfqws2/ipset-sites.list` — одна строка на сервис.

```
17 */6 * * * /opt/etc/nfqws2/ipset-update.sh >/dev/null 2>&1
```

Держите список сервисов коротким. Каждый адрес из `ipset.list` обрабатывается независимо
от SNI, поэтому сервис, которому обход не нужен, добавляет только нагрузку и риск.
`youtube.com` в примерах отсутствует намеренно: его адреса тянут за собой общую
инфраструктуру Google и ломают соседние сервисы. Всё, что обрабатывать не надо, идёт
в `ipset_exclude.list` — он имеет приоритет над `ipset.list`.

Скрипт заменяет файл только если результат выглядит осмысленно: скачалась хотя бы
половина сервисов, получено не меньше 100 адресов, и список не усох больше чем вдвое
против прежнего. Иначе он пишет причину в `/opt/var/log/ipset-update.log` и оставляет
старый файл нетронутым. Предыдущая версия сохраняется рядом как `ipset.list.prev`.
Перезапускать ничего не нужно — `nfqws2` перечитывает список сам, по времени изменения.

Загрузка идёт через `wget` (пакет `wget-ssl` и так нужен для установки), при его
отсутствии — через `curl`. На OpenWRT пути без префикса `/opt`, скрипт определяет их сам.
