...
В режиме `$MODE_AUTO` домен попадает в `auto.list` после трёх неудач за 60 секунд и
остаётся там навсегда — сам `nfqws2` записи не удаляет. Со временем список набирает
мусор: одноразовые имена сессий, домены, которые уже есть в `user.list` или
`exclude.list`, и домены второго уровня, чьи поддомены и так курируются вручную
(оставить такой apex — значит распространить десинк на всё, что под ним).

В комплекте есть `/opt/etc/nfqws2/auto-janitor.sh`, который такие записи убирает:

```bash
/opt/etc/nfqws2/auto-janitor.sh -n     # показать вердикты, ничего не менять
/opt/etc/nfqws2/auto-janitor.sh        # применить и послать демону SIGHUP
```

Перед изменением он делает копию `auto.list.janitor-prev`, а решения пишет в
`/opt/var/log/nfqws2-janitor.log`. Свой список доменов на выброс можно вести в
`/opt/etc/nfqws2/janitor-drop.list` — одна строка на домен, поддомены учитываются.

Оставшиеся записи скрипт считает в `/opt/etc/nfqws2/janitor-seen.tsv`. Смотреть там
надо на колонку `episodes` — сколько раз домен появлялся в `auto.list` заново. От трёх
и выше означает, что блокировка возвращается и домен пора переносить в `user.list`.
Колонка `runs` — это только возраст записи, сигналом она не является.

Запускать удобно раз в час через cron:

```
27 * * * * /opt/etc/nfqws2/auto-janitor.sh >/dev/null 2>&1
```

На OpenWRT пути без префикса `/opt`: сам скрипт лежит в `/etc/nfqws2/auto-janitor.sh`,
лог — в `/var/log/nfqws2-janitor.log`. Каталоги скрипт определяет сам, по своему
расположению, задавать ничего не нужно.
