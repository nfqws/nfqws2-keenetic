...
### Сначала исключите DNS

Фильтрующий или мёртвый DNS выглядит со стороны точно так же, как сломанный обход:
часть сайтов перестаёт открываться, остальное работает. Проверить это — секунды,
а перебор стратегий при испорченном DNS не поможет никогда.

```bash
/opt/etc/nfqws2/dns-check.sh
```

Скрипт показывает, какие резолверы используются (на Keenetic — ещё и список апстримов
из конфига DNS-прокси), и делает две проверки: отвечает ли резолвер на заведомо
существующие имена, и не выдумывает ли он адрес для имени, которого не существует.
Второе — признак перехвата NXDOMAIN, то есть фильтрующего DNS, чьим ответам нельзя
доверять и по остальным именам. Код возврата 1, если что-то из этого сработало.

Сравнения адресов с публичным резолвером в скрипте намеренно нет: любой CDN законно
отдаёт разным резолверам разные адреса, и такая проверка давала ложную тревогу почти
на каждом имени. Проверки достижимости адресов тоже нет — `nc` в busybox собран без
`-z` и `-w`, а `timeout` в образе отсутствует, так что «недоступен» означал бы отказ
инструмента, а не адреса.
